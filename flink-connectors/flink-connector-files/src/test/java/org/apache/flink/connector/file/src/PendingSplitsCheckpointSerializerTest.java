...
public class PendingSplitsCheckpointSerializerTest {
	...
	private static void assertCheckpointsEqual(PendingSplitsCheckpoint expected, PendingSplitsCheckpoint actual) {
		assertEquals(expected.getSplits().size(), actual.getSplits().size());

		final Iterator<FileSourceSplit> expectedIter = expected.getSplits().iterator();
		final Iterator<FileSourceSplit> actualIter = actual.getSplits().iterator();
		while (expectedIter.hasNext()) {
			final FileSourceSplit expectedSplit = expectedIter.next();
			final FileSourceSplit actualSplit = actualIter.next();
			FileSourceSplitSerializerTest.assertSplitsEqual(expectedSplit, actualSplit);
		}
	}
}
